"""Training loop for the 11B MoE LLM — A100 80GB optimized.

Key optimizations vs 1B version:
  - bitsandbytes 8-bit AdamW (halves optimizer memory)
...
def load_data(processed_dir):
    train = np.load(processed_dir / "train.npy", mmap_mode="r")
    val = np.load(processed_dir / "val.npy", mmap_mode="r")
    return train, val


def get_batch(data, batch_size, device):
    idx = np.random.randint(0, data.shape[0], size=batch_size)
    batch = np.asarray(data[idx])
...
def evaluate(model, val_data, batch_size, device, dtype):
    ...
    for i in range(0, n_eval, batch_size):
        batch = np.asarray(val_data[idx[i : i + batch_size]])
        x = torch.from_numpy(batch[:, :-1].astype(np.int64)).to(device)
        y = torch.from_numpy(batch[:, 1:].astype(np.int64)).to(device)
...
def train(cfg: Config):
    ...
    print(f"8-bit optimizer: {tcfg.use_8bit_optimizer}")
    ...
    if tcfg.use_8bit_optimizer and device.type == "cuda":
    ...
    if device.type == "cuda":
        ...
        print(f"Memory after model+optimizer: {mem_alloc:.1f} GB / 80 GB")

    use_wandb = tcfg.use_wandb
    if use_wandb:
        import wandb
        wandb.init(project="moe-11b-bilingual", config={"model": mcfg.__dict__, "training": tcfg.__dict__})
    ...
    pbar = tqdm(total=tcfg.max_steps, initial=start_step, desc="Training")
    while step < tcfg.max_steps:
        ...
        if step % tcfg.log_interval == 0:
            ...
            if use_wandb:
                wandb.log({"train/loss": avg_loss, "train/aux": avg_aux, "train/lr": lr, "train/tok_s": tps, "train/step": step})
        ...
        if step % tcfg.eval_interval == 0:
            ...
            print(f"\n  >> eval loss: {eval_loss:.4f} | ppl: {ppl:.2f} | GPU mem: {gpu_mem:.1f}GB")
            log_file.write(f"step {step} | train_loss {avg_loss:.4f} | eval_loss {eval_loss:.4f} | ppl {ppl:.2f}\n")
            log_file.flush()
            if use_wandb:
