"""Tokenize raw text data and pack into fixed-length sequences (multiprocessing)."""
import sys
import argparse
import numpy as np
    ...
    with Pool(n_workers, initializer=_init_worker, initargs=(str(sp_model_path),)) as pool:
        for f in input_files:
            ...
            with open(f, "r") as fh:
                lines = fh.readlines()
            results = list(
                tqdm(pool.imap(_tokenize_line, lines, chunksize=1000), total=len(lines), desc=f"Tokenizing {f.name}")
            )
            for ids in results:
...
if __name__ == "__main__":
    ...
    # Auto-discover ALL .txt files in the raw directory (pretrain + distill copies)
    input_files = sorted(f for f in raw_dir.glob("*.txt") if f.name != "tokenizer_sample.txt")
    ...
    tokenize_files(input_files, Path(args.tokenizer), args.seq_len, args.val_ratio, Path(args.out_dir))
