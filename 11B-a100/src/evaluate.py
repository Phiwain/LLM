"""Evaluate the 11B MoE model: bilingual perplexity and generation samples."""
import argparse
import math
import numpy as np
import torch
...
def eval_perplexity(model, data, batch_size, device, dtype):
    ...
    for i in range(0, n, batch_size):
        batch = np.asarray(data[idx[i : i + batch_size]])
...
@torch.no_grad()
def generate(model, sp, prompt, max_tokens=100, temperature=0.8, device="cuda"):
    ids = [1] + sp.encode(prompt, out_type=int)
...
def main(cfg, checkpoint, tokenizer_path):
    ...
    if (processed / "val_en.npy").exists() and (processed / "val_fr.npy").exists():
        ...
        print(f"\n{'='*60}")
        print(f"Bilingual Evaluation — 11B MoE")
        print(f"{'='*60}")
        print(f"English  — loss: {loss_en:.4f} | perplexity: {ppl_en:.2f}")
        print(f"Francais — loss: {loss_fr:.4f} | perplexity: {ppl_fr:.2f}")
        print(f"{'='*60}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", type=str, default=str(PROJECT_ROOT / "checkpoints" / "final.pt"))
    parser.add_argument("--tokenizer", type=str, default=str(PROJECT_ROOT / "tokenizer" / "bpe.model"))
