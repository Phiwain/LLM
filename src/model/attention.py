"""Multi-Head Attention with RoPE, causal masking, and optional Flash Attention."""
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, cos_sin: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        q = self.wq(x).view(B, S, self.n_heads, H).transpose(1, 2)  # (B,H,S,d)
        ...
        if self.use_flash and _HAS_SDPA:
            # PyTorch native scaled_dot_product_attention (uses Flash Attention on A100)
